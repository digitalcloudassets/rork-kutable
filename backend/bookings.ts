...
app.post('/api/bookings/reschedule', async c => {
  ...
  if (!bookingId || !newStartISO) return c.json({ error: 'bookingId and newStartISO required' }, 400);
  const supa = getAdminClient();
  if (!supa) return c.json({ error: 'Database not configured' }, 503);
  ...
  const { error } = await supa.from('bookings').update({ start_utc: start.toISOString(), end_utc: end.toISOString(), status: 'rescheduled' }).eq('id', bookingId);
  if (error) return c.json({ error: error.message }, 500);
  return c.json({ ok: true });
});
